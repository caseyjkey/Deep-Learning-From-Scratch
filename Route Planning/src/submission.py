...
class ShortestPathProblem(SearchProblem):
    ...
    def successorsAndCosts(self, state: State) -> List[Tuple[str, State, float]]:
        successors = []
        for location in self.cityMap.distances[state.location]:
            successors.append((location, State(location, state.memory), self.cityMap.distances[state.location][location]))
        return successors
...
def getStanfordShortestPathProblem() -> ShortestPathProblem:
    ...
    startLocation, endTag = "5802970977", "amenity=parking_entrance"
...
class WaypointsShortestPathProblem(SearchProblem):
    ...
    def startState(self) -> State:
        return State(self.startLocation, tuple(sorted(list(set(self.waypointTags) & set(self.cityMap.tags[self.startLocation])))))

    def isEnd(self, state: State) -> bool:
        return self.endTag in self.cityMap.tags[state.location] and self.waypointTags == state.memory

    def successorsAndCosts(self, state: State) -> List[Tuple[str, State, float]]:
        successors = []
        for location in self.cityMap.distances[state.location]:
            memory = tuple(sorted(list(set(state.memory) | set(self.waypointTags) & set(self.cityMap.tags[location]))))
            successors.append((location, State(location, memory), self.cityMap.distances[state.location][location]))
        return successors
...
def getStanfordWaypointsShortestPathProblem() -> WaypointsShortestPathProblem:
    ...
    startTag = "landmark=oval"
    startLocation = "5802970977"
    waypointTags = ["amenity=food", "landmark=AOERC"]
...
def aStarReduction(problem: SearchProblem, heuristic: Heuristic) -> SearchProblem:
    class NewSearchProblem(SearchProblem):
        ...
        def successorsAndCosts(self, state: State) -> List[Tuple[str, State, float]]:
            return [(action, succState, cost + heuristic.evaluate(succState) - heuristic.evaluate(state)) for action, succState, cost in problem.successorsAndCosts(state)]
...
class StraightLineHeuristic(Heuristic):
    ...
    def __init__(self, endTag: str, cityMap: CityMap):
        ...
        # Precompute
        self.distances = {}
        self.endLocation = self.cityMap.geoLocations[locationFromTag(endTag, self.cityMap)]
        for location in self.cityMap.geoLocations:
            self.distances[location] = computeDistance(self.cityMap.geoLocations[location], self.endLocation)
        # ### START CODE HERE ###
        # ### END CODE HERE ###
...
class NoWaypointsHeuristic(Heuristic):
    ...
    def __init__(self, endTag: str, cityMap: CityMap):
        ...
        class ReverseShortestPathProblem(SearchProblem):
            ...
            def successorsAndCosts(
                ...
                if state.location == "END":
                    return [(location, State(location, None), 0) for location in cityMap.geoLocations if endTag in cityMap.tags[location]]
                else:
                    return [(location, State(location, None), cityMap.distances[state.location][location]) for location in cityMap.distances[state.location]]
                # ### START CODE HERE ###
                # ### END CODE HERE ###
        ...
        self.ucs.solve(ReverseShortestPathProblem())
