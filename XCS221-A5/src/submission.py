...
############################################################
# Problem 0
# Hint: Take a look at the CSP class and CSP examples in util.py
...
def create_nqueens_csp(n = 8):
    """
    Return an N-Queen problem on a board of size |n| * |n|.
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            csp.add_binary_factor(variables[i], variables[j], 
                                 lambda col1, col2, rd=row_diff: col1 != col2 and abs(col1 - col2) != rd)
    return csp
    ...
    def add_quarter_constraints(self, csp):
        ...
        for request in self.profile.requests:
            if len(request.quarters) > 0:
                for quarter in self.profile.quarters:
                    var = (request, quarter)
                    csp.add_unary_factor(var, lambda cid: cid is None or quarter in request.quarters)
    ...
    def add_unit_constraints(self, csp):
        ...
        for quarter in self.profile.quarters:
            ...
            csp.add_unary_factor(
                total_units_var,
                lambda total:
                    self.profile.minUnits <= total <= self.profile.maxUnits
            )
