...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### START CODE HERE
  scores = outside_vectors.dot(center_word_vec)
  ...
  # Gradient with respect to center word vector dJ/dv_c = Sum(yHat_w * u_w) - u_o
  grad_center_vec[:] = probs.dot(outside_vectors) - outside_vectors[outside_word_idx]
  
  # Gradient with respect to outside vectors dJ/dU = v_c(yHat - y)^T
  # Create y vector where one value is 1 others 0
  y = np.zeros(len(probs))
  y[outside_word_idx] = 1
  
  # Update outside vectors' gradients
  for w in range(len(probs)):
    grad_outside_vecs[w] = center_word_vec * (probs[w] - y[w])
  ### END CODE HERE
...
def skipgram(current_center_word, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for outside_word in outside_words:
    ...
    loss_current, gradc, grado = word2vec_loss_and_gradient(
...
def word2vec_sgd_wrapper(word2vec_model, word2ind, word_vectors, dataset, window_size, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for i in range(batchsize):
    ...
    c, gin, gout = word2vec_model(center_word, context, word2ind, center_word_vectors,outside_vectors, dataset, word2vec_loss_and_gradient)
    loss += c / batchsize
