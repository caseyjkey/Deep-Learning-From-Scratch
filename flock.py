...
Updated for X on March 1, 2025 by Casey Key, Oracle Applications Developer
Fight On!
'''
...
class FlockX(object):
    ...
    def __init__(self, json_creds, output, cont):
        ...
        self._output = output # Output type ('csv' or 'db')
        ...
        self._streamer = Client('en-US').login(**self._creds)
    ...
    def start(self, quiet=True):
        ...
        try:
            for index, track in enumerate(self.tracks):
                ...
                stream = await self._streamer.search_tweet(track, 'Top')

        except (KeyboardInterrupt, SystemExit):
            end = time.time()
            print("\nSaved", len(tweets), "tweets over", end-start, "seconds.")
            cursor = con.cursor()
...
if __name__ == '__main__':           
    ...
    stream = FlockX(creds, outfile, samesearch)
